...
var tests = TestCatalog.Create(
    ...
    new TestCase("Bearer token normalization", TestBearerNormalization),
    new TestCase("Token extraction from cookie", TestCookieTokenExtraction),
    new TestCase("Test catalog rejects duplicate names", TestCatalogRejectsDuplicateNames),
    new TestCase("Test catalog snapshots input", TestCatalogSnapshotsInput),
    new TestCase("Lowest available authorized group", TestLowestAvailableGroup),
    new TestCase("User rate override", TestUserRateOverride),
    new TestCase("Availability threshold", TestAvailabilityThreshold),
    new TestCase("Provider warnings deserialize", TestProviderWarningsDeserialize),
    new TestCase("Null provider warnings are tolerated", TestNullProviderWarningsAreTolerated),
    new TestCase("Provider last-call aliases deserialize", TestProviderLastCallAliasesDeserialize),
    new TestCase("Twenty-minute provider metrics use medians", TestRollingProviderMetricsUseMedians),
    new TestCase("Rolling provider metrics discard samples outside the window", TestRollingProviderMetricsDiscardExpiredSamples),
    new TestCase("Rolling provider metrics clear account history", TestRollingProviderMetricsClearHistory),
    new TestCase("Rolling provider metrics are stable for duplicate groups", TestRollingProviderMetricsAreStableForDuplicateGroups),
    new TestCase("Adaptive constants match supplied economics", TestAdaptiveConstants),
    new TestCase("Adaptive preference follows interval boundaries", TestAdaptivePreferenceBoundaries),
    new TestCase("Current-group interval uses latest provider call", TestCurrentGroupIntervalResolution),
    new TestCase("Missing call time retains base preference", TestMissingCallTimeRetainsBasePreference),
    new TestCase("Adaptive penalty uses new multiplier", TestAdaptivePenalty),
    new TestCase("Adaptive completion time includes TTFT", TestAdaptiveCompletionTime),
    new TestCase("Adaptive net saving subtracts context penalty", TestAdaptiveNetSaving),
    new TestCase("Adaptive cost accepts positive saving", TestAdaptiveCostAcceptsPositiveSaving),
    new TestCase("Adaptive cost rejects slow candidate", TestAdaptiveCostRejectsSlowCandidate),
    new TestCase("Adaptive balanced requires all safeguards", TestAdaptiveBalancedSafeguards),
    new TestCase("Adaptive speed accepts generation boost", TestAdaptiveSpeedAcceptsGenerationBoost),
    new TestCase("Adaptive speed accepts end-to-end gain", TestAdaptiveSpeedAcceptsEndToEndGain),
    new TestCase("Balanced deadline uses explicit output budget", TestBalancedDeadlineUsesExplicitOutputBudget),
    new TestCase("Balanced deadline keeps current feasible node", TestBalancedDeadlineKeepsCurrentNode),
    new TestCase("Balanced deadline switches to cheapest feasible node", TestBalancedDeadlineChoosesCheapestFeasibleNode),
    new TestCase("Balanced deadline cold start chooses cheapest feasible node", TestBalancedDeadlineColdStart),
    new TestCase("Balanced deadline honors user soft tolerance", TestBalancedDeadlineHonorsSoftTolerance),
    new TestCase("Balanced deadline zero falls back to economy", TestBalancedDeadlineZeroFallsBackToEconomy),
    new TestCase("Adaptive short task is protected outside cost", TestAdaptiveShortTaskProtection),
    new TestCase("Adaptive invalid performance cannot switch", TestAdaptiveInvalidPerformance),
    new TestCase("Adaptive invalid old performance cannot satisfy relative time", TestAdaptiveInvalidOldPerformance),
    new TestCase("Warning provider remains eligible", TestWarningProviderRemainsEligible),
    new TestCase("Latest unavailable state remains ineligible", TestLatestUnavailableStateRemainsIneligible),
    new TestCase("Warning presentation excludes server message", TestWarningPresentationExcludesServerMessage),
    new TestCase("Warning decoration requires routable latest state", TestWarningDecorationRequiresRoutableLatestState),
    new TestCase("Routing presentation preserves availability threshold", TestRoutingPresentationPreservesAvailabilityThreshold),
    new TestCase("Routing presentation rejects invalid effective rate", TestRoutingPresentationRejectsInvalidEffectiveRate),
    new TestCase("Stale status rejection", TestStaleStatusRejection),
    new TestCase("Routing preferences default to Win32-compatible values", TestRoutingPreferenceDefaults),
    new TestCase("Routing preferences roundtrip", TestRoutingPreferenceRoundtrip),
    new TestCase("Balanced mode buys meaningful latency", TestBalancedModeBuysLatency),
    new TestCase("Balanced mode keeps price for moderate speed gap", TestBalancedModeKeepsPriceForModerateSpeedGap),
    new TestCase("Economy mode protects price", TestEconomyModeProtectsPrice),
    new TestCase("Speed mode accepts larger price premium", TestSpeedModeAcceptsLargerPremium),
    new TestCase("Selective policy preserves local routing weights", TestSelectivePolicyPreservesLocalWeights),
    new TestCase("Cost mode proposes strict cheapest candidate", TestCostModeProposesCheapest),
    new TestCase("Cost mode falls back when cheapest TTFT is unknown", TestCostModeFallsBackWhenCheapestTtftIsUnknown),
    new TestCase("Economy remains strict during frequent calls", TestFrequentCallsOverrideEconomy),
    new TestCase("Idle calls override speed with cost", TestIdleCallsOverrideSpeed),
    new TestCase("Adaptive rejection keeps current group", TestAdaptiveRejectionKeepsCurrentGroup),
    new TestCase("Adaptive acceptance updates selected Keys", TestAdaptiveAcceptanceUpdatesKeys),
    new TestCase("Adaptive traversal finds an accepted candidate beyond weighted winner", TestAdaptiveTraversalFindsAcceptedCandidateBeyondWeightedWinner),
    new TestCase("Adaptive rankings follow accepted algorithm order", TestAdaptiveRankingsFollowAcceptedAlgorithmOrder),
    new TestCase("Initial and invalid routes recover immediately", TestAdaptiveRecoveryBypassesGuard),
    new TestCase("Missing latency ranks last", TestMissingLatencyRanksLast),
    new TestCase("Invalid measurements are excluded", TestInvalidMeasurementsAreExcluded),
    new TestCase("Extreme latency scores stay finite", TestExtremeLatencyScoresStayFinite),
    new TestCase("Zero multiplier remains free", TestZeroMultiplierWindow),
    new TestCase("Initial route has an explainable reason", TestInitialRouteDecision),
    new TestCase("Preview and simulation share the initial route decision", TestPreviewAndSimulationShareInitialDecision),
    new TestCase("Adaptive speed winner switches after guard", TestWeightedSpeedWinnerSwitchesImmediately),
    new TestCase("Already optimal route does not switch", TestAlreadyOptimalRouteDecision),
    new TestCase("Invalid current route switches", TestInvalidCurrentRouteDecision),
    new TestCase("No candidate keeps route state", TestNoCandidateDecision),
    new TestCase("Route state persists atomically", TestRouteStateRoundtrip),
    new TestCase("Unreadable route state resets safely", TestUnreadableRouteStateResets),
    new TestCase("Dry run never updates a Key", TestDryRunNeverUpdatesKey),
    new TestCase("Account data is cached but monitor data is fresh", TestAccountDataCache),
    new TestCase("Routing result exposes cached user rates", TestRoutingResultExposesUserRates),
    new TestCase("Forced refresh bypasses account cache", TestForcedAccountRefresh),
    new TestCase("Business authentication failure retries once", TestBusinessAuthenticationRetry),
    new TestCase("Network failure never triggers login", TestRoutingNetworkFailureDoesNotLogin),
    new TestCase("Explicit empty Key selection is rejected", TestRoutingRejectsEmptySelection),
    new TestCase("Successful updates persist target state", TestSuccessfulRoutePersistsState),
    new TestCase("Partial update failure clears route certainty", TestPartialFailureClearsState),
    new TestCase("Already optimal cycle reports selected Keys", TestAlreadyOptimalReportsSelectedKeys),
    new TestCase("Mixed selected groups reconcile to target", TestMixedSelectedGroupsReconcile),
    new TestCase("Audit log writes valid JSON and rotates safely", TestAuditLogWritesValidJsonAndRotates),
    new TestCase("Publish script checks native exit codes", TestPublishScriptChecksNativeExitCodes),
    new TestCase("Encrypted settings roundtrip", TestEncryptedSettingsRoundtrip),
    new TestCase("Usable access token is reused", TestUsableAccessTokenIsReused),
    new TestCase("Expired access token refreshes first", TestExpiredAccessTokenRefreshesFirst),
    new TestCase("Rejected refresh falls back to login", TestRejectedRefreshFallsBackToLogin),
    new TestCase("Refresh API code falls back to login", TestRefreshApiCodeFallsBackToLogin),
    new TestCase("Refresh network failure does not log in", TestRefreshNetworkFailureDoesNotLogIn),
    new TestCase("Authentication API code is classified", TestAuthenticationApiCodeIsClassified),
    new TestCase("Login endpoint maps session", TestLoginEndpointMapsSession),
    new TestCase("Refresh endpoint maps rotated session", TestRefreshEndpointMapsRotatedSession),
    new TestCase("Refresh keeps token when server omits rotation", TestRefreshKeepsTokenWhenServerOmitsRotation),
    new TestCase("Authentication error hides server message", TestAuthenticationErrorHidesServerMessage),
    new TestCase("Business error hides server message", TestBusinessErrorHidesServerMessage),
    new TestCase("Authentication statuses have safe diagnostics", TestAuthenticationStatusesHaveSafeDiagnostics),
    new TestCase("Business authentication statuses keep business diagnostics", TestBusinessAuthenticationStatusesKeepBusinessDiagnostics),
    new TestCase("Malformed authentication responses retain endpoint context", TestMalformedAuthenticationResponsesRetainEndpointContext),
    new TestCase("Unknown errors do not expose credential text", TestUnknownErrorsDoNotExposeCredentialText),
    new TestCase("Interactive login requirement is rejected", TestInteractiveLoginRequirementIsRejected),
    new TestCase("Empty key selection roundtrips", TestEmptyKeySelectionRoundtrips),
    new TestCase("First key selection chooses first active key", TestFirstKeySelectionChoosesFirstActiveKey),
    new TestCase("Initialized empty key selection stays empty", TestInitializedEmptyKeySelectionStaysEmpty)
);
...
static void TestCatalogSnapshotsInput()
{
...
static void TestRollingProviderMetricsUseMedians()
{
...
static void TestRollingProviderMetricsDiscardExpiredSamples()
{
    var now = new DateTimeOffset(2026, 7, 22, 9, 0, 0, TimeSpan.Zero);
    var window = new ProviderMetricsRollingWindow();
...
static void TestRollingProviderMetricsClearHistory()
{
    var now = new DateTimeOffset(2026, 7, 22, 9, 0, 0, TimeSpan.Zero);
    var window = new ProviderMetricsRollingWindow();
...
static void TestRollingProviderMetricsAreStableForDuplicateGroups()
{
    var now = new DateTimeOffset(2026, 7, 22, 9, 0, 0, TimeSpan.Zero);
    var window = new ProviderMetricsRollingWindow();
