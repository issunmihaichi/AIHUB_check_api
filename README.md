...
The v1.1.13 rules below supersede the older interval-only description later in
...
- **Policy switches:** every normal Economy, Balanced, or Speed change uses a
  ...
  `ManualOverride` bypass these guards.
...
Economy 选择严格最低有效倍率；Balanced 使用 `P90(TTFT) + 输出预算 / P25(输出速度)` 判断 26.73 秒硬 Deadline 和用户软容忍度；用户选择 Speed 时，传统自适应规则仍要求生成速度提升超过 20% 且涨价不超过 10%，或整体快 30 秒以上且不涨价。所有正常策略切换（包括 Economy 降价、Balanced Deadline/最快回退）都经过 30 秒、6 次已完成评估和第三次同目标提案门槛；初始路由、当前分组硬失效恢复和手动强制分组不受阻塞。检查在手动刷新、模拟、立即路由或自动轮询取得新供应商数据时执行。
